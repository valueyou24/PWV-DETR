...
class WiseIouLoss(torch.nn.Module):
    '''Wise-IoU with Vertical structure conformity factor (V-WIoU)

    :param monotonous: {
            None: origin V1
            True: monotonic FM V2
            False: non-monotonic FM V3 + Vertical-aware Omega_v (V-WIoU)
        }

    V-WIoU introduces a learnable aspect ratio threshold (tau) and a vertical
    structure conformity factor (Omega_v) into the non-monotonic dynamic
    focusing mechanism, suppressing harmful gradients from predicted boxes whose
    morphology deviates from the longitudinal anatomical prior of the esophagus.

    Reference:
        Omega_v = exp(-mu * max(0, W_p/H_p - tau))
        L = (beta / (delta * alpha^(beta - delta))) * Omega_v * L_base
    '''
    momentum = 1e-2
    alpha = 1.7
    delta = 2.7
    mu = 5.0  # Scaling coefficient controlling morphological penalty strength (Eq. 13)
    ...
    def __init__(self, ltype='WIoU', monotonous=False, inner_iou=False, focaler_iou=False):
        ...
        # Learnable aspect ratio threshold tau (Eq. 13)
        # Initialized to 0.98; converges to ~0.68 (consistent with esophageal lesion morphology)
        self.register_parameter('tau', torch.nn.Parameter(torch.tensor(0.98)))
    ...
    def _scaled_loss(self, loss, iou=None):
        if isinstance(self.monotonous, bool):
            ...
            if self.monotonous:
                ...
                # --- V-WIoU: Vertical structure conformity factor Omega_v (Eq. 13) ---
                # Compute predicted box aspect ratio W_p / H_p from xyxy coordinates
                pred_xyxy = self['pred'][..., :4]
                pred_w = pred_xyxy[..., 2] - pred_xyxy[..., 0]  # x2 - x1
                pred_h = pred_xyxy[..., 3] - pred_xyxy[..., 1] + 1e-7  # y2 - y1
                aspect_ratio = pred_w / pred_h  # W_p / H_p

                # Omega_v = exp(-mu * max(0, W_p/H_p - tau))
                # Penalizes boxes whose aspect ratio deviates from the vertical prior
                omega_v = torch.exp(-self.mu * torch.nn.functional.softplus(aspect_ratio - self.tau))

                # r_v = (beta / (delta * alpha^(beta - delta))) * Omega_v
                divisor = self.delta * torch.pow(self.alpha, beta - self.delta)
                loss *= beta / divisor * omega_v
        return loss
    ...
    def __repr__(self):
        return f'{self.__name__}(iou_mean={self.iou_mean.item():.3f}, tau={self.tau.item():.3f})'
