...
from ultralytics.utils.loss import FocalLoss, VarifocalLoss, SlideLoss, EMASlideLoss, SlideVarifocalLoss, \
    EMASlideVarifocalLoss, MALoss
from ultralytics.utils.metrics import bbox_iou, bbox_inner_iou, bbox_focaler_iou, bbox_mpdiou, bbox_inner_mpdiou, \
...
class DETRLoss(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_wiseiou:  #修改版的V-WIoU
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=''):
        ...
        if self.nwd_loss:
            ...
            loss[name_giou] = self.iou_ratio * (loss[name_giou].sum() / len(gt_bboxes)) + (1.0 - self.iou_ratio) * (
                        (1.0 - nwd).sum() / len(gt_bboxes))
        else:
